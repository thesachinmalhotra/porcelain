...
function connectConfig(mapping: string) {
  return { input: { generate: { interval: "100ms", mapping, count: 0 } }, pipeline: { processors: [] }, output: { drop: {} } }
}
...
async function createStore(id: string) {
  ...
  await store.createWithRevision({ id, name: "Smoke pipeline", metadata: { source: "sac-46-smoke" }, desiredConfig: connectConfig('root = "v1"') })
  return store
}
...
run("SAC-46 real Connect publish", () => {
  it("creates a missing stream through POST and projects runtime stats", async () => {
    ...
    await expect(client.getStream(id)).resolves.toMatchObject({ active: true, config: { input: { generate: { mapping: 'root = "v1"' } }, output: { drop: {} } } })
  }, 30000)
  ...
  it("updates an existing stream through PUT and observes the restarted runtime", async () => {
    ...
    await expect(client.getStream(id)).resolves.toMatchObject({ active: true, config: { input: { generate: { mapping: 'root = "v2"' } } } })
  }, 30000)

  it("preserves the native Connect error when Streams API publish is rejected", async () => {
    const id = "sac-46-invalid-" + Date.now(); streamIds.push(id); const client = createConnectClient({ baseUrl })
    await expect(client.createStream(id, { input: { generate: { interval: "1s", mapping: 'root = "invalid"', count: 0 } }, output: { definitely_not_a_real_output: {} } })).rejects.toSatisfy((error: unknown) =>
      error instanceof ConnectRequestError && error.status === 400 && error.details?.message?.includes("unable to infer output type") === true,
    )
  }, 30000)
})
